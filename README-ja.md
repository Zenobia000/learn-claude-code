...
どこから始めればいいかわからない？以下の 3 フェーズガイドで素早く習得できる。

### フェーズ 1：コア (s01-s03) — 約 1-2 時間

| 順序 | ファイル | 学べること |
|------|----------|------------|
| 1 | `agents/s01_agent_loop.py` | **最小の Agent ループ** — 1 つのループ + Bash ツール = Agent |
| 2 | `agents/s02_tool_use.py` | **ツール拡張** — ループは変わらない；新ツールは dispatch map に登録するだけ |
| 3 | `agents/s03_todo_write.py` | **計画能力** — 先にステップを列挙し、次に実行；完了率が倍増 |

この 3 ファイルで Claude Code の骨格を理解できる：**モデルが決定し、Harness が実行する。**

### フェーズ 2：高度なメカニズム (s04-s07) — 約 2-3 時間

| 順序 | ファイル | 学べること |
|------|----------|------------|
| 4 | `agents/s04_subagent.py` | **サブエージェント** — 大きなタスクを分解、各サブタスクは独立した context を使用 |
| 5 | `agents/s05_skill_loading.py` | **オンデマンド知識** — tool_result で注入、system prompt を膨張させない |
| 6 | `agents/s06_context_compact.py` | **コンテキスト圧縮** — 3 層の圧縮戦略でコンテキスト溢れを解決 |
| 7 | `agents/s07_task_system.py` | **タスクシステム** — ファイルベースのタスクグラフ + 依存関係 |

### フェーズ 3：マルチエージェント協調 (s08-s12) — 約 2-3 時間

| 順序 | ファイル | 学べること |
|------|----------|------------|
| 8 | `agents/s08_background_tasks.py` | バックグラウンド実行、Agent はブロックされない |
| 9 | `agents/s09_agent_teams.py` | マルチエージェントチーム + 非同期メールボックス |
| 10 | `agents/s10_team_protocols.py` | チーム通信プロトコル |
| 11 | `agents/s11_autonomous_agents.py` | 自律的タスク獲得 |
| 12 | `agents/s12_worktree_task_isolation.py` | worktree 分離による並列実行 |

### 最小実行パス

時間が極めて限られている場合、**s01 → s04 → s07**（ループ → サブエージェント → タスクシステム）だけ読めば Claude Code の設計の 80% を把握できる。その後 `agents/s_full.py` で完全統合版を確認。
